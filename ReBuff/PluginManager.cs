...
namespace ReBuff
{
    public class PluginManager : IPluginDisposable
    {
        ...
        private ReBuffConfig Config { get; init; }
            ...
            try
            {
                if (ImGui.Begin("ReBuff_Root", _mainWindowFlags))
                {
                    if (this.Config.VisibilityConfig.IsVisible(true))
                    {
                        ...
                        foreach (WidgetListItem widget in this.Config.WidgetList.Widgets)
                        {
                            widget.Draw((viewPortSize / 2) + this.Config.GroupConfig.Position);
                        }
                    }
                }
            }
