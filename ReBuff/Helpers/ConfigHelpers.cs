...
namespace ReBuff.Helpers
{
    public static class ConfigHelpers
    {
        private static readonly JsonSerializerSettings _serializerSettings = new JsonSerializerSettings
        {
            ...
            SerializationBinder = new ReBuffSerializationBinder()
        };
        ...
        public static ReBuffConfig LoadConfig(string path)
        {
            ReBuffConfig? config = null;
            ...
            try
            {
                if (File.Exists(path))
                {
                    ...
                    config = JsonConvert.DeserializeObject<ReBuffConfig>(jsonString, _serializerSettings);
                }
            }
            catch (Exception ex)
            {
                ...
                if (File.Exists(path))
                {
                    try
                    {
                        ...
                        PluginLog.Information($"Backed up ReBuff config to '{backupPath}'.");
                    }
                    catch
                    {
                        PluginLog.Warning($"Unable to back up ReBuff config.");
                    }
                }
            }

            return config ?? new ReBuffConfig();
        }

        public static void SaveConfig()
        {
            ConfigHelpers.SaveConfig(Singletons.Get<ReBuffConfig>());
        }

        public static void SaveConfig(ReBuffConfig config)
        {
            try
            {
        ...
        public override object? ReadJson(JsonReader reader, Type objectType, object? existingValue, JsonSerializer serializer)
        {
            ...
            try
            {
                ...
                if (label is not null &&
                    Singletons.IsRegistered<ReBuffConfig>() &&
                    ...
                    if (!FontsManager.ValidateFont(fontOptions, label.FontID, label.FontKey))
                    {
                        ...
                        foreach (string installedFont in installedFonts)
                        {
                            if (fontName.ToLower().Equals(installedFont.ToLower()))
                            {
                                ...
                                Singletons.Get<ReBuffConfig>().FontConfig.AddFont(fontData);
                                break;
                            }
                        }
                    }
    ...
    /// </summary>
    public class ReBuffSerializationBinder : ISerializationBinder
    {
        ...
        private static List<Type> _configTypes = new List<Type>()
        {
            ...
            typeof(WidgetIcon),
            typeof(WidgetLabel),
            typeof(WidgetListItem),
            typeof(WidgetListConfig),
        ...
        public ReBuffSerializationBinder()
        {
