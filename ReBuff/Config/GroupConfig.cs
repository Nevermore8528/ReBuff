...
namespace ReBuff.Config
{
    public class GroupConfig : IConfigPage
    {
        ...
        [JsonIgnore] public int _WidgetCount = 0;
        ...
        public void DrawConfig(IConfigurable parent, Vector2 size, float padX, float padY)
        {
            if (ImGui.BeginChild("##GroupConfig", new Vector2(size.X, size.Y), true))
            {
                ...
                if (ImGui.Button("Sort", new Vector2(60, 0)))
                {
                    if (parent is WidgetGroup g)
                    {
                        g.SortIcons(_iconPos, _iconPos, _recusiveSort, _conditionsSort, _WidgetCount);
                    }
                }
                ...
                if (_keepsorted)
                {
                    if (parent is WidgetGroup g)
                    {
                        g.SortVisible(_iconPos, _iconPos, _recusiveSort, _conditionsSort, _WidgetCount);
                    }
                }

                if (parent is WidgetGroup group)
                {
                    ImGui.NewLine();
