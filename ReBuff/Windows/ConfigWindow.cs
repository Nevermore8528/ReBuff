    ...
    public class ConfigWindow : Window
    {
        ...
        private void DrawNavBar(IConfigPage? openPage, Vector2 size, float padX)
        {
            ...
            if (ImGui.BeginChild($"##{this.WindowName}_NavBar", new Vector2(size.X, NavBarHeight), true))
            {
                ...
                if (_configStack.Peek() is WidgetListItem widget)
                {
                    offset -= 80;
        ...
        public override void OnClose()
        {
            ...
            var config = Singletons.Get<ReBuffConfig>();
            foreach (WidgetListItem widget in config.WidgetList.Widgets)
            {
                widget.StopPreview();
            }
